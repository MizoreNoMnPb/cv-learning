...
交叉注意力（Cross-Attention）用一个序列产生查询，从另一个序列的键和值中读取信息。本文重点解释两个序列的角色、输出形状与计算量。缩放点积和多头的基础推导见 [注意力机制](../01-Foundations/attention.md)。
...
缩放因子控制点积的尺度，其方差推导依赖独立性等假设，详见 [注意力机制第 2.3 节](../01-Foundations/attention.md#attention-scaling)。
